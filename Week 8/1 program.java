...
class payRoll extends Employee implements allowance{
	...
	{
		grossSalary = basicSalary + (basicSalary * dearnessAllow) +(basicSalary *
				houseRentAllow)+ (cityAllow) ;
		}
	}
class MultipleInheritanceDemo{
	public static void main(String args[])
	{
		payRoll virat = new payRoll("Virat kohli",128966,45678.0d);
		virat.displayEmployeeDetails();
		payRoll suresh = new payRoll("Roith sharma",129966,25678.0d);
